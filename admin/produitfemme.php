...
class produitfemme {
 ...
 public function insert($conn){
     $st=$conn->prepare("insert into produitfemme values(default,?,?,?)");
     $st->execute(array($this->nom,$this->prix,$this->image));

    }
 public function selection($conn){
   
     $st=$conn->prepare("select * from produitfemme");
      $st->execute();
     return $st;
 ...
 public function selection_id($conn,$id){
     //requete parametrée
     $st=$conn->prepare("select * from produitfemme where ID=?");
     $st->execute(array($id));
     $row=$st->fetch();
     $this->nom=$row['nom'];
     $this->prix=$row['prix'];
 ...
 public function update ($conn,$id){
     ...
     $st=$conn->prepare("update produitfemme set nom=?,prix=?,image=? where ID=?");
 ...
 public function supprimer($conn,$id){
     $st=$conn->prepare("delete from produitfemme where ID=?");
     $st->execute(array($id));
 }
}
